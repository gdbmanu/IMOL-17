...
From a variational perspective, the passing from $q(Z) \simeq p(Z|\boldsymbol{x}, \boldsymbol{u})$ to $q'(Z) \simeq p(Z|\boldsymbol{x}, \boldsymbol{u}, \boldsymbol{x}', \boldsymbol{u}')$ is the \emph{posterior update}, and the change from $F(\boldsymbol{x}|\boldsymbol{u})$ toward   $F(\boldsymbol{x}|\boldsymbol{u}', \boldsymbol{x}', \boldsymbol{u})$ is the \emph{cost update}. From there comes the possibility to reduce the cost (or ``surprise'') from additional sampling in a model-based approach.
Considering now the variational posterior $q'$ \emph{before} its update (meaning in short that $q'$ is initialized at $q$ --~the preceding posterior estimate~-- before considering the new data),
then $\text{KL}(q(Z)||p(Z|\boldsymbol{x}',\boldsymbol{u}',\boldsymbol{x}, \boldsymbol{u}))$ is said the \emph{posterior update cost} (PUC), representing how ``far'' the initial $q$ is from the final $q'$ \emph{before} the posterior update.
...
with $q^{(n)}$ the shared (or cumulative) encoding according to the past and current observations, and $q^{(n-1)}$ having the role of the prior, providing a \emph{forward} variational encoding scheme (see also \cite{chung2015recurrent}, and \cite{fraccaro2016sequential} for a backward implementation).  
