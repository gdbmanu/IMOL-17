...
$F$ is also called the variational Free Energy for it is mathematically analog to the Helmhotz Free Energy \cite{friston2010free}.
...
&= F(\boldsymbol{x}|\boldsymbol{u}, \boldsymbol{x}', \boldsymbol{u}')\nonumber
\end{align}
with $q'(\boldsymbol{z}) \simeq p(\boldsymbol{z}|\boldsymbol{x}, \boldsymbol{u}, \boldsymbol{x}', \boldsymbol{u}') = \frac{p(\boldsymbol{x}|\boldsymbol{z}, \boldsymbol{u})p(\boldsymbol{z}|\boldsymbol{x}', \boldsymbol{u}')}{p(\boldsymbol{x}|\boldsymbol{u},\boldsymbol{x}',\boldsymbol{u}')}
= \frac{p(\boldsymbol{x}'|\boldsymbol{z}, \boldsymbol{u}')p(\boldsymbol{z}|\boldsymbol{x}, \boldsymbol{u})}{p(\boldsymbol{x}'|\boldsymbol{u}',\boldsymbol{x},\boldsymbol{u})}$ after optimization the shared posterior.
%(or shared \emph{code}). 
...
From a variational perspective, the passing from $q(Z) \simeq p(Z|\boldsymbol{x}, \boldsymbol{u})$ to $q'(Z) \simeq p(Z|\boldsymbol{x}, \boldsymbol{u}, \boldsymbol{x}', \boldsymbol{u}')$ is the \emph{posterior update}, and the change from $F(\boldsymbol{x}|\boldsymbol{u})$ toward   $F(\boldsymbol{x}|\boldsymbol{u}', \boldsymbol{x}', \boldsymbol{u})$ is the \emph{cost update}. From there comes the possibility to reduce the cost (or ``surprise'') from adequate sampling in a model-based approach \cite{friston2012perceptions}.
{\color{blue} Imagine now the variational posterior $q'$ initialized at $q$ --~the preceding posterior estimate~-- before update. Then $\text{KL}(q(Z)||p(Z|\boldsymbol{x}',\boldsymbol{u}',\boldsymbol{x}, \boldsymbol{u}))$ --~see eq. (\ref{eq:FEP-uxu2})~-- represents the \emph{posterior update cost} (PUC), telling how ``far'' the initial $F$ is from the final description length $-\log p(\boldsymbol{x}| \boldsymbol{u}, \boldsymbol{x}', \boldsymbol{u}') $ \emph{before} the posterior update.}

In eq.~(\ref{eq:FEP-uxu}), the posterior $p(Z|\boldsymbol{x}',\boldsymbol{u}')$ plays the role of the prior, as in eq. (\ref{eq:FEP-prior-u}). This reflects the chaining of the posterior to the role of the prior in subsequent inference steps, a classical property of sequential Bayesian inference \cite{wald1945sequential}.
...
following a minimal free-Energy path \cite{friston2012perceptions}. %\cite{friston2015active}.
...
%It is moreover consistent with a uniform prior distribution over the latent space, which somehow allows to ``escape the dark room'' without structured priors.
