...
%so that the latent state space (the cause of the visual field) now decomposes in an uncontrolled emitter $\boldsymbol{z}$ and an explicit motor command $\boldsymbol{u}$. 
...
\end{align}
with $F(\boldsymbol{x}'|\boldsymbol{u}', \boldsymbol{x}, \boldsymbol{u})$ the \emph{conditional} Free Energy, a quantity approaching  $-\log p(\boldsymbol{x}'| \boldsymbol{u}', \boldsymbol{x}, \boldsymbol{u})$, which is the approximate conditional natural Shannon Information provided by seeing $\boldsymbol{x}'$ at $\boldsymbol{u}'$ after seeing $\boldsymbol{x}$ at $\boldsymbol{u}$. 
%We call it here the \emph{conditional Free Energy} (for it is conditioned over $\boldsymbol{x}, \boldsymbol{u}$)

Then, noting that the conditional Shannon Information can be approached by: 
\begin{align}
...
\label{eq:FEP-uxu}
\end{align}
{\color{blue} which is a special case of the POMDP\footnote{Partially Observed Markov Decision Processes.} variational Free Energy formulated in \cite{friston2015active}}.
...
&= \mathbb{E}_{q^{(n-1)},p}\left[A(X, \boldsymbol{u}, q^{(n-1)})\right]  \\
