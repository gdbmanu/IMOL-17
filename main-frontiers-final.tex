...
\begin{align}
q^{(n)}(Z) \propto p(Z|\boldsymbol{x},\boldsymbol{u}) p(Z|\boldsymbol{x}',\boldsymbol{u}') ... p(Z|\boldsymbol{x}^{(n)},\boldsymbol{u}^{(n)}) \label{eq:accum}
\end{align}
...
\begin{align} 
q^{(n)}(Z) \propto p(Z|\boldsymbol{x},\boldsymbol{u}) q^{(n-1)}(Z) \label{eq:accum-post}
\end{align}
...
So a single probabilistic framework allows for describing both the model-free and mode-based view to the scene decoding setup\footnote{Note the Information theoretic estimators provided in previous section and in the following one are compatible with the POMDP setup, but restrained to the ``three-party'' case --~see eq.~(\ref{eq:three-party})~-- for mathematic simplicity.}. 
Equipped with a probabilistic model, important quantities can then be defined. 
One such quantity is the \emph{Bayesian surprise} \cite{itti2005bayesian} defined as the Kullback-Leibler divergence between an actual view $\boldsymbol{x}$ and a model $\boldsymbol{z}$. In the ``bottom-up'' case they consider, only local statistics $q$ are formed over small image patches of a given image $\Omega$. We note here $\boldsymbol{u}$ the index of a patch and $q(\boldsymbol{z}|\boldsymbol{x},\boldsymbol{u})$ the inferred features from the data actually observed at $\boldsymbol{u}$. For each patch $\boldsymbol{u}$, the divergence between the actual data and the model is:
$$ S(\boldsymbol{x}, \boldsymbol{z}| \boldsymbol{u}) = \text{KL}(q(\boldsymbol{z}| \boldsymbol{x}, \boldsymbol{u})||q(\boldsymbol{z}))$$
which shares a formal similarity with the second term of the variational Free Energy (see eq.\ref{eq:FEP-prior-u}). 
...
Its strong explanatory power and scalability explain its straightforward application in image and video compression  \cite{wang2003foveation,guo2010novel}.

The Itti and Baldi's approach to vision constitutes however a challenge to modelers. It is indeed task independent and, in contrast with perception elementary requirements, totally unaware of the scene interpretation. It thus provides little explanation about the sight-orientation cognitive operations. 
...
More formally, the Bayesian surprise term enters in contradiction (or in ``dialectic'' contrast) with the scene encoding setup, where one objective of the encoder is to minimize the divergence between the inferred code and the prior expectations.
...
$$v(\boldsymbol{x}, \boldsymbol{u}, q^{(n-1)}) = \text{KL}(q^{(n)}||q^{(n-1)})$$
...
\begin{enumerate}
	\item $v(\boldsymbol{x}, \boldsymbol{u}, q^{(n-1)})=q^{(n)}(\hat{\boldsymbol{z}})$
	 ...
	 the ``accuracy'' of posterior in \cite{najemnik2005optimal}.
...
Another quantity considered in many considered in  
...
A baseline model of action is


This approach to inference is called \emph{active sampling}, for the choice of $\boldsymbol{u}$ determines the sensory sample $\boldsymbol{x}$ that is observed, conditioning the final posterior estimate.
...
The task turns here to minimize the number of samples so as to provide an accurate estimate. 
...
More elaborate strategies consider the past observations $\{\boldsymbol{u}_{1,..., t-1}, \boldsymbol{x}_{1,..., t-1}\}$ to choose the most promising action $\boldsymbol{u}_t$ (regarding next posterior accuracy). The problem turns out to design  a \emph{controller} $C$. A controller is a function that, given a context $\{\boldsymbol{u}_{1:t-1}, \boldsymbol{x}_{1:t-1}\}$, sets up an action $\boldsymbol{u} = C(\boldsymbol{u}_{1:t-1}, \boldsymbol{x}_{1:t-1})$. The role of the controller is not to achieve a goal-oriented task, but to render the estimation of the outer state more accurate. The controller is said \emph{perception-driven}. 
...
\begin{align}
\hat{\boldsymbol{u}} = \underset{\boldsymbol{u}\in\mathcal{U}}{\text{ argmin }}  \ell(\boldsymbol{u})
\end{align}
