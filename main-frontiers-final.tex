...
is a code that is both compact and efficient at restoring the data. 
If $\boldsymbol{x}$ is the original data, the corresponding code $\boldsymbol{z}$ is generated by a distribution $q$, i.e. $\boldsymbol{z} \sim q(Z)$. This distribution is called the \emph{encoder}. Then, the reconstruction is made possible with a second conditional probability over the codes, i.e. $p(X|\boldsymbol{z})$. If $\boldsymbol{z}$ is the current code, the reconstructed data is $\tilde{\boldsymbol{x}} \sim p(X|\boldsymbol{z})$. 

The reconstruction accuracy is estimated by an information-theoretic quantity, the so-called ``description length'', that is consistent with Shannon's Information, i.e:
$$H(\boldsymbol{x}) = -\log \text{Pr}(\boldsymbol{x}) \simeq - \log \sum_{\boldsymbol{z} \in \mathcal{Z}} p(\boldsymbol{x},\boldsymbol{z})  \leq - \sum_{\boldsymbol{z} \in \mathcal{Z}} q(\boldsymbol{z}) \log \frac{p(\boldsymbol{x},\boldsymbol{z})}{q(\boldsymbol{z})}$$
with $\mathcal{Z}$ an arbitrary encoding space and $q$ an arbitrary distribution over the codes. 

In practice, the quantity to minimize is:
\begin{align}
F(\boldsymbol{x}) &= - \sum_{\boldsymbol{z} \in \mathcal{Z}} q(\boldsymbol{z}) \log \frac{p(\boldsymbol{x}|\boldsymbol{z}) \rho(\boldsymbol{z})}{q(\boldsymbol{z})}\nonumber\\
&= - \mathbb{E}_{\boldsymbol{z}\sim q} \log p(\boldsymbol{x}|\boldsymbol{z}) + \text{KL}(q||\rho)
\label{eq:FEP}\end{align}
also called the variational free energy, that is composed of two terms, namely the reconstruction accuracy $- \mathbb{E}_{\boldsymbol{z}\sim q} \log p(\boldsymbol{x}|\boldsymbol{z})$ and the Kullback-Leibler divergence $\text{KL}(q||\rho)$ between the current code $q$ and a reference distribution $\rho$. 
...
The viewpoint selection setup introduces an additional factor $\boldsymbol{u}$. The data $\boldsymbol{x}$ that is actually read is now conditioned on $\boldsymbol{u}$, so that:
\begin{align}
F(\boldsymbol{x}|\boldsymbol{u}) 
&= - \mathbb{E}_{\boldsymbol{z}\sim q} \log p(\boldsymbol{x}|\boldsymbol{z}, \boldsymbol{u}) + \text{KL}(q||\rho)
