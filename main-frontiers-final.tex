...
\end{align}
with $-\log p(\boldsymbol{x}'| \boldsymbol{u}', \boldsymbol{x}, \boldsymbol{u})$ the approximate \emph{conditional} natural Shannon Information provided by seeing $\boldsymbol{x}'$ at $\boldsymbol{u}'$ after seeing $\boldsymbol{x}$ at $\boldsymbol{u}$, and $F(\boldsymbol{x}'|\boldsymbol{u}', \boldsymbol{x}, \boldsymbol{u})$ the \emph{conditional} Free Energy, 
%We call it here the \emph{conditional Free Energy} (for it is conditioned over $\boldsymbol{x}, \boldsymbol{u}$)
with the conditional Shannon Information approached by the conditional Free Energy as: 
\begin{align}
...
\end{align}
so that\footnote{It must be noted that the conditional Free Energy
	...
	- \text{KL}(q(Z|\boldsymbol{x}, \boldsymbol{u})||p(Z))$$
	with the first term the ``extrinsic value'' and the second term the ''epistemic value''. Compare with eq.~(\ref{eq:cond-F-KL}) for the KL sign inversion.}:
\begin{align}
...
Here, contrary to the previous case
...
\end{align}
with $\text{KL}(q^{(n-1)}(Z)||p(Z|\boldsymbol{x}, \boldsymbol{u}))$ the divergence between the \emph{past} cumulated posterior and the \emph{present} posterior, measured at viewpoint $\boldsymbol{u}$ only. This divergence is here interpreted as the Free Energy \emph{margin}, i.e. the difference between the \emph{present} conditional Free Energy and 
