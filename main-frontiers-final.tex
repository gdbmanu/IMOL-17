...
p(Z|\boldsymbol{x}', \boldsymbol{u}')p(Z|\boldsymbol{x}, \boldsymbol{u})$ the shared (or cumulative) encoding with $p(Z|\boldsymbol{x}, \boldsymbol{u})$ playing the role of the prior.
 
It is thus noteworthy that the variational encoding approach provides estimators of information theoretic quantities that can further on be used in optimization, e.g. for defining the objective function of a controller. 
%, i.e. maximizing the mutual information between different sensors or different sensory fields).
{\color{blue} Recursive encoder / Iterated codes : the recursive encoder setup is consistent with the POMDP setup}.
It also provides a way to implement recursive (or cumulative) encoders that share consistency with the posterior update in the POMDP setup (at the difference that variational encoders are \emph{model-free} though the POMDP setup is \emph{model-based}).
